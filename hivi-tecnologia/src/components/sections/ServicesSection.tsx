...
import { Brain, Headphones, Code2, Zap } from 'lucide-react';
import { FEATURED_SERVICES, SECONDARY_SERVICES, type ServiceKey } from '@/constants/services';
...
export function ServicesSection({ showHeading = true }: { showHeading?: boolean }) {
  ...
  const renderCard = (key: ServiceKey, isFeatured: boolean, index: number) => {
    const Icon = SERVICE_ICONS[key];
    return (
      <AnimateOnScroll key={key} delay={Math.min(index % 2, 1) * 80}>
        <div
  ...
  return (
    <section className="bg-white py-[88px]" aria-label={t('services.sectionTitle')}>
      <div className="mx-auto max-w-[1200px] px-6 sm:px-8">
        ...
        {/* Nível 1 — Serviços em destaque */}
        <div className="grid grid-cols-1 gap-[22px] sm:grid-cols-2">
          {FEATURED_SERVICES.map((key, index) => renderCard(key, true, index))}
        </div>

        {/* Nível 2 — Outras soluções */}
        <AnimateOnScroll>
          <div className="mt-12 mb-6">
            <h3 className="text-[22px] font-bold text-[#0f1b2d]">
              {t('services.secondaryTitle')}
            </h3>
            <div className="mt-2 h-0.5 w-10 rounded-full bg-[#2563eb]" aria-hidden="true" />
          </div>
        </AnimateOnScroll>
        <div className="grid grid-cols-1 gap-[22px] sm:grid-cols-2">
          {SECONDARY_SERVICES.map((key, index) => renderCard(key, false, index))}
        </div>
      </div>
    </section>
  );
